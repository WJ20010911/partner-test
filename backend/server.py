...
def init_db():
    ...
    conn.execute(
        ...
        ("test_uploader", "test@local.dev", "", "user", datetime.utcnow().isoformat() + 'Z')
    )
    conn.commit()
    conn.close()
...
def handle_register(body):
    ...
    try:
        ...
        conn.execute(
            ...
            (uid, email, hash_password(password), "contributor", datetime.utcnow().isoformat() + 'Z', username)
        )
        conn.commit()
...
def handle_create_question(headers, body):
    ...
    try:
        ...
        conn.execute(
            ...
            (qid, content, json.dumps(options, ensure_ascii=False), dimension, weight, time_limit, user["uid"], datetime.utcnow().isoformat() + 'Z')
        )
...
def handle_submit_test(headers, body):
    ...
    try:
        for ans in answers:
            ...
            if behavior == "complaint":
                ...
                conn.execute(
                    ...
                    (sid, qid, datetime.utcnow().isoformat() + 'Z')
                )
        ...
        conn.execute(
            ...
            (rid, json.dumps(answers, ensure_ascii=False), surface_score, real_score, token, datetime.utcnow().isoformat() + 'Z')
        )
        conn.commit()
        return json_response({
            "record_id": rid,
...
def handle_record_skip(headers, body):
    ...
    try:
        ...
        conn.execute(
            ...
            (sid, question_id, reason, datetime.utcnow().isoformat() + 'Z')
        )
        conn.commit()
...
def record_admin_attempt(ip):
    _admin_login_attempts.setdefault(ip, []).append(datetime.utcnow().timestamp())
